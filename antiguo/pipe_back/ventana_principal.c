...
*/
void establecer( /*pipeline_t * pipeline, */ ventana_principal_t *
		ventana_principal);
...
*/
    ...
    */
void guardar_como_aux(ventana_principal_t * ventana_principal,
		      const char *archivo);
...
void on_nuevo2_activate(GtkMenuItem * menuitem, gpointer user_data)
{
	...
	mostrar(GTK_STATUSBAR(ventana_principal->statusbar1), 0,
...
void on_establecer_error(GtkMenuItem * menuitem, gpointer user_data)
{
    ventana_principal_t *ventana_principal =
	(ventana_principal_t *) user_data;
    gint destino = elegir_modulo(ventana_principal->window1,
...
// GtkWidget *fixed = ventana_principal->fixed1;
    propiedades(-1,
		/*GTK_FIXED(fixed), ventana_principal->pipeline, &ventana_principal->modificado */
		ventana_principal);
	...
	g_signal_connect((gpointer) ventana_principal->botones	/*pipeline->
								   m_elemento */
			 [ventana_principal->pipeline->m_numero -
			  1].m_widget, "toggled", G_CALLBACK(pinchado),
			 ventana_principal);
	establecer( /*ventana_principal->pipeline, */ ventana_principal);
    }
}
			...
			GdkEvent * event, gpointer user_data)
{
    ventana_principal_t *ventana_principal =
	(ventana_principal_t *) user_data;
    return !salir(ventana_principal->window1,
		  ventana_principal->modificado);
}
...
void on_propiedades1_activate(GtkMenuItem * menuitem, gpointer user_data)
{
	...
	if (gtk_toggle_button_get_active
	    ...
	    propiedades(i, ventana_principal);
	}
    }
    if (ventana_principal->modificado) {
	gtk_widget_set_sensitive(ventana_principal->nuevo2, TRUE);
...
void on_cerrar_biblioteca_activate(GtkButton * button, gpointer user_data)
{
	...
	if (gtk_toggle_button_get_active
	    (GTK_TOGGLE_BUTTON(ventana_principal->botones
			       /*pipeline->m_elemento */ [i].m_widget))) {
...
void insertar( /*GtkFixed * fixed, */ ventana_principal_t * ventana)
{
	...
	gtk_fixed_put(GTK_FIXED(ventana->fixed1),
		      GTK_WIDGET(ventana->botones /*m_elemento */ [id].
				 m_widget),
...
void propiedades(gint id_elemento, ventana_principal_t * ventana)
{
	  ...
	  if (id_elemento == -1) {
	    ...
	    /*	    ventana->botones = (botones_t *) realloc(ventana->botones,
							 ...
							 *
							 ...
							 pipeline->
							 m_numero);*
		ventana->botones[ventana->pipeline->m_numero -
				 1].m_widget =
		  gtk_toggle_button_new_with_label(ventana->pipeline->m_elemento[ventana->pipeline->m_numero - 1].m_nombre);
		gtk_widget_show(ventana->
				botones[ventana->pipeline->m_numero -
					1].m_widget);
		...
		gtk_fixed_put(GTK_FIXED(ventana->fixed1),
			      GTK_WIDGET(ventana->
					 botones[ventana->pipeline->
						 m_numero - 1].m_widget),
			      ventana->botones[ventana->pipeline->
					       m_numero - 1].m_x,
			      ventana->botones[ventana->pipeline->
			      m_numero - 1].m_y);*/
	  } else {
	    ...
	    if(strcmp(ventana->pipeline->m_elemento[id_elemento].m_ruta, ruta_v)) {
	    ...
	    strcpy(ventana->pipeline->m_elemento[id_elemento].m_nombre, nombre_v);
...
void establecer(ventana_principal_t *ventana_principal)
{
    int i, si = 0, cual;
		      ...
		      const char *file)
{
	...
	ventana_principal->modificado = FALSE;
	mostrar(GTK_STATUSBAR(ventana_principal->statusbar1),
		ventana_principal->archivo, &ventana_principal->id);
	gtk_widget_set_sensitive(ventana_principal->guardar2, FALSE);
    }
}
...
int crear_timer(ventana_principal_t * ventana, guint retardo)
{
    ventana->m_timer = g_timeout_add(retardo, tick, ventana);
...
/*	char buf[23];
	...
	for (i = 0; i < ventana_principal->pipeline->m_numero; ++i) {
	    g_signal_connect((gpointer) ventana_principal->botones[i].
			     m_widget, "toggled", G_CALLBACK(pinchado),
			     ventana_principal);
	}
	establecer(ventana_principal /*->pipeline, ventana_principal*/ );
    }
