...
{
    dialogo_conectar_t *dialogo_conectar =
	(dialogo_conectar_t *) malloc(sizeof(dialogo_conectar_t));
...
#ifndef WIN32
    dialogo_conectar->dialog1_icon_pixbuf =
	gdk_pixbuf_new_from_file("pipeline.xpm", 0);
    if (dialogo_conectar->dialog1_icon_pixbuf) {
	gtk_window_set_icon(GTK_WINDOW(dialogo_conectar->dialog1),
			    dialogo_conectar->dialog1_icon_pixbuf);
	gdk_pixbuf_unref(dialogo_conectar->dialog1_icon_pixbuf);
    }
#endif

    dialogo_conectar->dialog_vbox1 =
	GTK_DIALOG(dialogo_conectar->dialog1)->vbox;
    ...
    gtk_box_pack_start(GTK_BOX(dialogo_conectar->dialog_vbox1),
		       dialogo_conectar->vbox1, TRUE, TRUE, 0);

    dialogo_conectar->label1 =
	gtk_label_new_with_mnemonic("_Elija un m\303\263dulo:");
    gtk_widget_set_name(dialogo_conectar->label1, "label1");
    gtk_widget_show(dialogo_conectar->label1);
    gtk_box_pack_start(GTK_BOX(dialogo_conectar->vbox1),
		       dialogo_conectar->label1, FALSE, FALSE, 0);

    dialogo_conectar->comboboxentry1 = gtk_combo_box_new_text();
    gtk_widget_set_name(dialogo_conectar->comboboxentry1,
			"comboboxentry1");
    gtk_widget_show(dialogo_conectar->comboboxentry1);
    gtk_box_pack_start(GTK_BOX(dialogo_conectar->vbox1),
		       dialogo_conectar->comboboxentry1, TRUE, TRUE, 0);

    dialogo_conectar->dialog_action_area1 =
	GTK_DIALOG(dialogo_conectar->dialog1)->action_area;
    gtk_widget_set_name(dialogo_conectar->dialog_action_area1,
			"dialog_action_area1");
    ...
    dialogo_conectar->cancelbutton1 =
    ...
    gtk_dialog_add_action_widget(GTK_DIALOG(dialogo_conectar->dialog1),
				 dialogo_conectar->cancelbutton1,
				 GTK_RESPONSE_CANCEL);
    ...
    gtk_dialog_add_action_widget(GTK_DIALOG(dialogo_conectar->dialog1),
				 dialogo_conectar->okbutton1,
				 GTK_RESPONSE_OK);
    GTK_WIDGET_SET_FLAGS(dialogo_conectar->okbutton1, GTK_CAN_DEFAULT);

    gtk_label_set_mnemonic_widget(GTK_LABEL(dialogo_conectar->label1),
				  dialogo_conectar->comboboxentry1);
