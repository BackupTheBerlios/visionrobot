...
void RedNeuronalBorrar(red_neuronal_t **red)
{
    if(red && (*red)) {
...
void CalculaCapa(/*red_neuronal_t * red, */int num1, int num2, double* capa1, double* capa2, double* pesos)
{
  double sum;
  capa1[0]=1;
  int j;
...
{
  CalculaCapa(red->numEntrada, red->numOculta, red->capaEntrada, red->capaOculta, red->pesosEntrada);
  CalculaCapa(red->numOculta, red->numSalida, red->capaOculta, red->capaSalida,red->pesosOculta);
}
...
red_neuronal_t* Abrir(const char* fileName)
{
   ...
   red_neuronal_t * redA= RedNeuronalCrear(ne,no,ns);
...
void CargaInputImagen(red_neuronal_t * red, char * dibujo, int ancho, int alto)
{
  int k=0;
  if(!red) return;
...
//  Imagen* imagen= new Imagen(dibujo,ancho,alto);
  ...
  CargaInputImagen(red, dibujo,ancho,alto);
  ComputarCapas(red);
  switch(tipo){
        case ORDEN:
