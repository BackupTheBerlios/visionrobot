...
\section{Detalles}
\begin{itemize}
  \item {\bf Entrada}: Este módulo no tiene entradas del pipeline. Sólo los datos de la imagene externa. Como argumentos, la función que debe realizar, y el tamaño y los bits de la imagen que genera.
  \item {\bf Salida}: Una estructura de datos como la que definimos en \ref{formato_imagenes}.
  \item {\bf Descripción}: Módulo para la captación de imágenes de cámara, archivos, vídeo o generadas automáticamente, que alimenta al pipeline.
\end{itemize}
...
\begin{itemize}
  ...
  \item{\bf Bytes}: Un entero que determina el número de bytes por punto.
  \item{\bf Buffer}: Un puntero (en la implementación de C, realmente implementa un vector) de número de 8 bits sin signo (1 byte, corresponde al formato {\tt unsigned char} en C), que contiene la información de colores de la imagen.
\end{itemize}
...
\begin {itemize}
\item {\bf DirectX}: Las bibliotecas {\em DirectX} nos han provisto de la interfaz necesaria en su módulo {\em DirectShow}, para la adquisición de imágenes de cámara. % TODO: esto no se sabe :P % como para la reproducción de un archivo de vídeo.
